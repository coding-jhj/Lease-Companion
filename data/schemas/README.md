...
- **`generated/` — 현재 canonical JSON Schema 생성본(2026-07-16).** `ai/src/lease_companion_ai/schemas/unified.py`의 Pydantic 모델에서 자동 생성한 6개(contract-context·document-extraction·input-snapshot·correction-request·analysis-run-result·generation-result). **손으로 수정하지 않는다.** 재생성 명령:
