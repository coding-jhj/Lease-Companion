...
- **상용 LLM 구조화 (`extraction`/`providers`)**: MVP 조항 유형·명확성 후보 구조화·필드 추출은 Gemini 3.5 Flash. `classification` 분리 경계는 다음 canonical schema 변경 때 적용한다.
...
src/lease_companion_ai/
  ...
  classification/ 추출과 분리할 조항 유형·명확성 후보 계약 결정 기록
