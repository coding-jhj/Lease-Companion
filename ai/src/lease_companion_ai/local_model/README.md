...
> **(선택) 상용 vs 로컬 성능비교 실험용 — MVP 크리티컬 패스 제외.** MVP 조항 구조화·명확성 후보는 상용 LLM(Gemini 3.5 Flash)이 담당한다.

파인튜닝한 **로컬 7B 모델**로 임대차 조항의 유형과 명확성 후보를 분류한다. 조항 유형·명확/불명확/확인 필요 후보·책임 주체·조건을 구조화한다. **최종 판정을 내리지 않는다.** 명시적 비교·판정은 `rules/`가 담당한다.
