...
- 구현 완료(배치 5): J01~J13은 별도 `JudgmentRetrievalQuery`를 사용하고 `judgment_spec.csv`의 판정별 source allowlist에 포함된 공식 청크만 근거로 연결한다. allowlist 원문이 없으면 J 판정과 시급도를 유지한 채 `evidence_sources=[]`로 남긴다.
