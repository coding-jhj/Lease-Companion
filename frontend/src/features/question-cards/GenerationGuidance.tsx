import type { RuleGuidanceDto } from "../../types/api";
...
export function GenerationGuidance({ items }: { items: RuleGuidanceDto[] }) {
  if (items.length === 0) return null;
  return (
    <section className="guidance-list" aria-labelledby="guidance-title">
      <h2 id="guidance-title">확인 질문과 다음 행동</h2>
      {items.map((item) => (
        <article className="guidance-card" key={item.rule_id}>
