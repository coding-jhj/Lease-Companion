...
function ActionList({ title, description, items }: { title: string; description: string; items: string[] }) {
  return (
  ...
  return (
    <section className="action-hub" aria-labelledby="action-hub-title">
      ...
      <div className="action-hub__grid">
        <ActionList title="먼저 물어볼 질문" description="입금이나 서명 전에 상대방에게 확인하세요." items={questions} />
        <ActionList title="서명 전 확인 행동" description="문서와 조건을 직접 대조할 항목입니다." items={signingActions} />
        <ActionList title="계약 직후 행동" description="임차권을 지키기 위해 이어서 처리하세요." items={postActions} />
