...
// 한 조항 안의 하위항목을 줄바꿈으로 분리해 긴 조항을 읽기 쉽게 만든다. 원문 글자는 그대로
// 두고 줄바꿈과 번호 뒤 한 칸만 넣는다. CSS `white-space: pre-wrap`과 함께 쓴다.
// 나누는 자리는 다섯 곳뿐이다.
//
// 1. 문장 끝("~다.") 뒤 — 기호 없이 여러 약속이 이어 붙은 특약을 문장 단위로 나눈다.
//    종결어미 앞 음절을 함께 요구해 목록 표시 "다."(가·나·다)는 건드리지 않는다.
// 2. 항 번호 앞 — ①②③ 원문자와 "1 " 형태를 모두 본다. 본문 속 교차참조(제6조의3 제1항)나
//    금액 숫자는 앞 글자 조건 때문에 걸리지 않는다.
// 3. "1." 형태의 번호 항목 앞 — 관리비 세부항목("1.일반관리비 2.전기료 … 7.TV")처럼 점 뒤에
//    바로 내용이 붙는 목록. 소수점·전화번호·날짜는 점 뒤가 숫자라 걸리지 않는다.
// 4. "예:" 항목 라벨 앞 — 수선 비용 부담처럼 표의 여러 행이 한 줄로 합쳐져 나온다
//    ("임대인부담  예: …  임차인부담  예: …"). 라벨과 "예:" 사이 칸 구분이 행 경계다.
// 5. 닫는 괄호 뒤 — 표준계약서 체크박스 항목은 "( … )"로 끝나고 다음 항목 이름이 이어진다.
//    항목 사이 유일한 경계라 여기서 나눠야 한 문단으로 뭉치지 않는다. 조건이 둘 붙는다.
//    - 여백이 정확히 1칸일 때만. 2칸은 표의 칸 구분이라 "차임(월세)  금 400,000 원정"처럼
//      라벨과 값을 갈라 놓으면 오히려 읽기 나빠진다.
//    - 체크박스나 칸 구분이 있는 서식 조항에서만. 일반 산문은 문장 중간 괄호가 흔해
//      ("… 02-1234-5678 (평일) 이다.") 문장이 잘린다.
const formClausePattern = /[□☑☐■▢]|[^\S\n]{2}/;

export function formatClauseText(text: string): string {
  const formatted = text
    .replace(/(?<=[가-힣]다\.)[ \t]+(?=[가-힣(（])/g, "\n")
    .replace(/(?<=[다.)])\s+(?=[1-9]\s+[가-힣])/g, "\n")
    .replace(/(?<=\S)[ \t]*(?=[①-⑳])/g, "\n")
    .replace(/(?<=\S)[ \t]+(?=[1-9]\d?\.\s*[가-힣A-Za-z])/g, "\n")
    .replace(/(?<=\S)[ \t]+(?=\S+[ \t]{2}예\s*:)/g, "\n");
  const separated = formClausePattern.test(text)
    ? formatted.replace(/(?<=\)) (?! )(?=[가-힣(（])/g, "\n")
    : formatted;
  // 서식 목록은 "1.일반관리비"처럼 번호와 내용이 붙어 나온다. 번호를 알아보게 한 칸 띄운다.
  return separated.replace(/(^|\n)([1-9]\d?\.|[①-⑳])(?=\S)/g, "$1$2 ");
}

// PDF 줄바꿈(\n)으로 잘게 끊긴 조항을 하나로 이어붙인 뒤, 조 번호(제N조)·특약 불릿(•)
// 기준으로만 다시 나눈다. 문장·단어 중간에서 끊기던 문제를 없앤다. 원래 조 단위면 그대로 복원된다.
// 항(①②③)은 카드를 쪼개지 않고 `formatClauseText`가 카드 안에서 줄바꿈한다 — 조 하나가
// 항 개수만큼 카드로 흩어지면 조 단위로 확인하기 어렵다.
