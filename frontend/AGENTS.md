...
`frontend/`는 **회원·계약 관리를 포함한 모바일 최적화 웹앱**이다. 로그인 사용자가 **계약 건 단위**로 관리하는 7단계 흐름을 화면으로 제공한다.

## 규칙

- 기준 사용자 흐름 7단계를 유지한다. (`docs/planning/user-flow.md`)
...
| `contract-create` | 2 | 계약 건 생성 |
| `document-upload` | 3 | 계약서·등기 등 문서 업로드 |
| `extraction-review` | 4 | AI 추출값 확인·수정과 계약 상황 입력 |
| `analysis-progress` | 5 | 분석 진행 상태 표시 |
| `result-report` | 6 | 판정·원문 증거·공식 근거·피해 유형 비교·질문·수정 요청·단계별 행동·전체 PDF 리포트 |
| `contract-detail` | 7 | 계약 건 상세, 저장된 체크리스트·계약 직후 행동 관리 |
