...
describe("ContractPreparationPage", () => {
  it("offers preparation actions without creating a contract", () => {
    ...
    expect(screen.getByRole("button", { name: /집을 볼 때 유의할 점/ })).toHaveAttribute("aria-pressed", "true");
    expect(screen.getByRole("checkbox", { name: "누수·곰팡이·결로·침수 흔적" })).toBeInTheDocument();
    fireEvent.click(document.querySelector('[aria-controls="preparation-step-2"]')!);
    expect(screen.getByText("가계약금을 보내기 전에 다음 자료를 먼저 요청하세요.")).toBeInTheDocument();
    expect(screen.getByRole("checkbox", { name: "등기사항증명서" })).toBeInTheDocument();
    expect(screen.getByRole("checkbox", { name: "계약서 초안 사본" })).toBeInTheDocument();
    expect(screen.getByRole("checkbox", { name: "특약사항" })).toBeInTheDocument();
    ...
    expect(document.querySelector('[aria-controls="preparation-step-3"]')).toBeInTheDocument();
  ...
  it("shows three preparation steps and keeps the first step active", () => {
    render(<MemoryRouter><ContractPreparationPage /></MemoryRouter>);

    const steps = screen.getAllByRole("button", { name: /완료$/ });
    expect(steps).toHaveLength(3);
    expect(steps[0]).toHaveAttribute("aria-pressed", "true");
    expect(steps[0]).toHaveTextContent("0 / 6 완료");
    expect(steps[1]).toHaveTextContent("0 / 3 완료");
    expect(steps[2]).toHaveTextContent("0 / 3 완료");
    expect(screen.getByLabelText("전체 0 / 12")).toBeInTheDocument();
  });

  it("updates the overall and step counts when an item is checked", () => {
    render(<MemoryRouter><ContractPreparationPage /></MemoryRouter>);

    fireEvent.click(screen.getByRole("checkbox", { name: "계약할 집의 정확한 주소와 동·호수" }));

    expect(screen.getByLabelText("전체 1 / 12")).toBeInTheDocument();
    expect(screen.getByRole("button", { name: /집을 볼 때 유의할 점 1 \/ 6 완료/ })).toBeInTheDocument();
  });
  ...
  it("shows a status message after copying the draft request", async () => {
    ...
    fireEvent.click(document.querySelector('[aria-controls="preparation-step-2"]')!);
    fireEvent.click(screen.getByRole("button", { name: "요청 문구 복사" }));
