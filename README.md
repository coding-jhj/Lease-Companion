...
- 회원가입·로그인·로그아웃·회원 탈퇴
- 사용자별 계약 건 생성·조회·삭제
- 내용 확인 화면에서 계약 유형·단계·지급·서명·날짜·대리인 상황 입력
...
2. 계약 대시보드·계약 건 생성
3. 계약서·등기 등 문서 업로드
4. 추출값 확인·수정 + 계약 상황 입력
5. 상용 LLM 구조화(Gemini 3.5 Flash)·규칙 엔진·RAG·상용 LLM 분석
6. 판정·원문 증거·공식 근거·질문·행동 리포트
7. 저장된 체크리스트·계약 직후 행동 관리

추출값 확인·수정과 계약 상황 입력은 분석(5단계) 전에 완료한다. 분석 결과·체크리스트는 계약 건 단위로 저장·재조회한다. 상세: [docs/planning/user-flow.md](docs/planning/user-flow.md)
...
- **MVP**: 회원·계약 건·업로드·확인 수정·J01~J13 판정·등기 교차검증·근거·질문·행동 리포트·저장 재조회·체크리스트·계약 직후 행동 상태 관리. 상세: [docs/planning/mvp-scope.md](docs/planning/mvp-scope.md)
...
- **실전 계약 점검**: 회원가입부터 계약 생성, 문서 업로드, 추출값 확인·수정, R01~R24·J01~J13 분석, DP01~DP08 비교, 특약별 공식 근거·질문·수정 요청·행동 안내, PDF, 체크리스트 저장·재조회까지 Backend·Frontend에 연결됐다.
