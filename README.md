...
| [`frontend/`](frontend/README.md) | React + TypeScript + Vite 기반 회원·계약 관리 모바일 웹앱 |
...
전체 MVP의 프론트엔드 스택은 React + TypeScript + Vite SPA로 확정됐다. DB 제품과 인증 구현 방식은 여전히 미정(TODO)이다. 환경변수가 필요한 후속 기능은 `.env.example`을 복사해 `.env`로 사용한다.
