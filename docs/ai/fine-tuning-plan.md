...
> **[2026-07-14 선정표 반영]** 로컬 7B 파인튜닝은 **MVP 크리티컬 패스에서 제외**됨(MVP 구조화는 상용 Gemini Flash). 이 계획은 **상용 vs 로컬 성능 비교(파인튜닝 B안) 병렬 실험**을 진행할 경우에만 실행한다. 우선순위 A안(상용+RAG) > B안(로컬+RAG) > C안(파인튜닝+RAG) 중 C안에 해당하며, GPU 지원 확인 후 착수 여부를 결정한다.
