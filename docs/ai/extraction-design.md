...
계약 문서에서 핵심 필드를 추출한다. 추출 전 문서 인식(`ingestion`)이 디지털 PDF 직접 추출(PyMuPDF·PDF.js)·스캔/사진 OCR(PaddleOCR-VL-1.6, VLM 통합)로 문서 내용을 확보한다.

## 인식·추출 흐름

1. **인식 (`ingestion`)**: 디지털 PDF는 직접 텍스트 추출(PyMuPDF·PDF.js), 스캔·사진 문서는 OCR(PaddleOCR-VL-1.6). VLM은 PaddleOCR-VL에 통합되어 별도 단계가 없다.
