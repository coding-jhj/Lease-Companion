...
- 공식 검증 출처 9개는 `data/rag/metadata/official_sources.jsonl` manifest로 고정했다. 자유이용이 확인된 법령 2개와 공공누리 제1유형인 법무부 표준 주택임대차계약서 1개는 정규화 원문과 SHA-256을 보존하고, 나머지 6개는 `metadata_only`로 둔다.
...
- 검색 가능한 로컬 원문은 법령 2개와 표준 주택임대차계약서 1개다. 나머지 정답 출처는 metadata-only이므로 목표값으로 보정하지 않고 실측값과 제한을 함께 기록한다.
...
- 표준 주택임대차계약서는 로컬 원문으로 검색할 수 있다. 체크리스트·등기 자료 등 metadata-only 출처만 허용된 J 항목은 기본 로컬 실행에서 `evidence_sources=[]`가 될 수 있으며, 검색 성공률을 임의로 보정하지 않는다.
...
- metadata-only 공식자료 6개 중 허용 원문을 추가한 뒤 retrieval 평가 재실행
