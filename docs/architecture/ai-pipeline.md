...
계약 문서 입력부터 저장까지의 단계별 흐름과 컴포넌트 책임을 정의한다. 각 단계 출력은 구조화 스키마(`ai/src/lease_companion_ai/schemas/` — **Pydantic 모델 단일 원본**, → [`../decisions/2026-07-16-shared-pydantic-schema.md`](../decisions/2026-07-16-shared-pydantic-schema.md))를 따르고, `pipelines`가 PoC·MVP 흐름을 각각 연결한다. 규칙 엔진은 R01~R10 축을 유지하면서 J01~J13 전체 판정을 별도 축으로 실행한다.

## 단계 흐름

```
문서 입력
  → 디지털 PDF 직접 추출(PyMuPDF·PDF.js) 또는 스캔 원본 Gemini VLM 1회 구조화
  → 핵심 필드 추출
  ...
  → 공식자료 RAG (근거)
  → 피해 유형 비교(DP01~DP08, 기존 판정의 표시용 묶음)
  → 검증된 유사 참고 사례 (공식 근거와 분리, corpus 없으면 빈 목록)
  → 저신뢰 결과 상용 LLM 재검토
...
| 단계 | 모듈(예상) | 책임 |
|------|-----------|------|
| 문서 검증·인식 | `ingestion` | 형식·크기·페이지·픽셀 제한 검증. 디지털 PDF는 텍스트 직접 추출, 스캔 PDF·사진은 VLM 경로로 분류 |
| 추출 | `extraction` | 디지털 텍스트 구조화 또는 스캔 원본에서 고정 Pydantic 필드를 Gemini 1회 호출로 직접 추출 |
...
| 피해 유형 비교 | `risk_patterns` | R/J 판정을 피해 유형 관점으로 결정적으로 묶음. 판정·시급도 변경 금지 |
| 상용 LLM 재검토·생성 | `generation` | 저신뢰 결과 재검토, 근거 기반 쉬운 설명·질문·수정 요청 문구·단계별 행동 생성 |
...
- **RAG는 근거만 검색한다.** `RuleStatus`·`urgency`는 규칙 엔진 결과를 유지한다. 근거가 없으면 `evidence_sources=[]`이며 설명 단계에서만 공식 근거 확인 필요를 표시한다.
