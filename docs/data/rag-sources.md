...
- 따라서 현재 로컬 원문 5개(법령 2개·법령 서식 1개·표준계약서 1개·체크리스트 1개)는 `distribution_mode=local_source`와 `content_sha256`을 기록하고, 나머지 4개는 `metadata_only`, `content_sha256=null`이다. `metadata_sha256`을 원문 해시로 가장하지 않는다.
