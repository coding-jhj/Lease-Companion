...
> schema_version **1.0.0** · 작성 2026-07-16 · 근거 ADR: [`../decisions/2026-07-16-shared-pydantic-schema.md`](../decisions/2026-07-16-shared-pydantic-schema.md)
...
실측 결과(2026-07-16, API 키 차단 로컬 실행): **112 passed, 3 warnings**. Schema·fixture 재생성 후 모델과 일치하며 CASE-001 rule goldset 자체검증 통과. 경고 3건은 Starlette/FastAPI 의존성 deprecation이다.
...
| `RuleResult` | 규칙 1개 판정 | **rule_id·rule_name·judgment_id·status·urgency·reason·question·recommended_actions·evidence_sources·limitations·completed** | (아래 파일 내부) |
...
**Enum 허용값**: `confidence` = `추출됨`·`불확실`·`실패`(숫자 거부) / `verification_status` = `unverified`·`confirmed`·`corrected` / `status` 9개·`urgency` 5개 = 루트 `AGENTS.md` 기준(R별 허용 status는 `data/rules/rule_spec.csv`와 일치하도록 모델이 검증) / `document_type` = `contract`·`registry`.
...
- 현재 `backend/app/schemas/contract.py`의 계약 유형·단계 Literal은 값은 통일됐지만 공통 Enum import 전이다. B가 새 저장 경계를 연결할 때 `ContractType`·`ContractStage`를 import하여 중복 정의를 제거한다.
...
- [ ] R01~R10 결과(`RuleResult` 11개 필드) 렌더링
...
| 단계 | 상태 |
|---|---|
| 1. A 패키지 준비 | **완료** (모델·실제 AI 경로 어댑터·Schema·fixture·테스트 112 passed) |
