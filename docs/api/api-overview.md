...
| `checklists` | 서명 전 체크리스트·계약 직후 행동 상태 관리 | ChecklistItem, PostContractAction | **구현됨**: `GET /api/contracts/{id}/checklist-items`(?kind=checklist/post_action 필터) · `PUT …/checklist-items/{kind}/{item_key}`(`{done}` upsert — 계약 건 단위 저장·재조회). 항목 문구·근거는 분석 결과가 원본이며 상태만 저장. item_key는 `R01:checklist:<hash>` / `R01:post_action:<hash>` 안정 식별자(→ `docs/decisions/2026-07-18-guidance-action-item-keys.md`). Backend는 형식과 kind 일치를 검증 |
