...
5. 상용 LLM 구조화·규칙 엔진·RAG·상용 LLM 분석
...
> 상태: 2026-07-26 승인된 합성 시나리오 3개가 AI 답변 평가, Backend 세션·턴·결과 저장 API, Frontend 대화·복기 화면과 연결됐다. 선택적 로컬 Supertonic 3·MuseTalk 1.5 미디어와 브라우저 Web Speech API 입력도 연결됐다. 실제 Gemini 네트워크 품질은 미검증이다. 기존 실전 계약 점검 7단계를 대체하지 않는다.

- 승인된 합성 계약 상황에서 가상 임대인·공인중개사와 확인 대화를 연습한다.
- 계약 규칙·공식자료 RAG·Guardrail은 재사용하되, 실제 계약 `contract_id`와 연습 세션·행동 평가는 분리한다.
- 조건부 보증금 반환, 제3자 계좌 송금, 대리인 권한 미확인 3개 시나리오에 행동 기반 복기와 공식 근거를 제공한다.
- 답변은 텍스트로 저장·평가한다. 지원 브라우저에서는 Web Speech API로 한국어 음성을 텍스트 입력란에 채운다. 선택적 로컬 TTS·립싱크 영상은 기능 플래그로 제공하고 실패 시 텍스트·기본 영상으로 복구한다.
...
- Python 규칙 엔진 문서 내부 판정과 문서 교차검증 (J01~J13)
- 등기사항증명서 교차검증
