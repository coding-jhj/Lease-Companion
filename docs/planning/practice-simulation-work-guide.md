...
- 기존 실패 2건을 제외한 AI 전체: `370 passed, 1 skipped, 2 deselected`
- Ruff: 통과
- mypy: 변경 소스 통과

## 10. 현재 알려진 기존 문제

다음 두 문제는 작업 1 이전과 작업 2 이후에 동일하게 확인됐습니다. 시뮬레이션 작업에서 임의로 수정하지 않았습니다.

### 1. RAG 원문 해시 불일치

`SRC-HTA-DECREE.txt`의 실제 SHA256과 공식자료 metadata의 `content_sha256`이 다릅니다. 이 때문에 전체 데이터 검사와 RAG manifest 테스트가 실패합니다.

### 2. 생성 평가 기대치 불일치

Offline generation 평가의 `trigger_coverage_rate` 실제값은 약 `0.979865`인데 테스트는 `1.0`을 기대합니다.

이 문서에서 `370 passed`라고 쓴 것은 위 두 기존 실패를 명시적으로 제외하고 나머지 전체 테스트를 실행한 결과입니다. 전체 테스트가 모두 통과했다고 표현하지 않습니다.
