...
- 현재 완료 범위: 작업 0~6
- 현재 브랜치: `main`
- 마지막 기능 커밋: 작업 5 커밋 `6c5df10 feat(practice): add backend session API`
- 다음 작업: 작업 7 — 세 시나리오 Frontend 화면·서비스·MSW
...
현재는 **시나리오 3개, 기존 R/J/DP 연결, 사용자 답변 평가, 누적 상태 머신, Gemini/Fake provider 선택, Backend 세션·턴·결과 저장 API와 전용 회귀 테스트까지 완성한 상태**입니다. Frontend 연결과 실제 Gemini 네트워크 smoke는 아직 완료되지 않았습니다.
...
| 작업 7 이후 | Frontend·E2E 연결 | 예정 | 미구현 |
...
Backend API 자동화 검증은 작업 6에서 완료했습니다. Frontend 화면과 실제 Frontend↔Backend E2E는 아직 연결하지 않았습니다.
...
Backend 모델·API·전용 API 테스트는 완료됐지만 Frontend 연결이 남아 있으므로 전체 연결 완료로 표시하지 않습니다.

## 16. 작업이 끝날 때마다 이 문서를 갱신하는 규칙
