...
- `PRACTICE-DEFERRED-REFUND-001`만 Python 대화 계획과 승인 fact 기반 Gemini NPC 대사 생성을 사용한다. 다른 시나리오는 승인 고정 대사를 유지한다.
...
- 별도 Gemini 대사 provider는 허용 fact를 공인중개사 말투로만 표현한다. 감정 상태는 사용하지 않는다.
- 생성 대사는 최대 2문장·100자이며, 검증 실패 시 한 번 재생성한 뒤 검수 fallback을 사용한다.
- NPC는 질문받은 사실만 답하고 임차인의 모범 행동을 제안하지 않는다.
...
후임 임차인 조건부 반환 NPC 대사:

```text
대상 시나리오 + key 있음 + PRACTICE_OFFLINE_MODE=false
→ GeminiPracticeDialogueProvider

그 외
→ speech_act별 검수 fallback
```

대사 생성 실패는 행동 평가 결과와 상태 전이를 바꾸지 않는다. 최종 검증 대사 또는 검수 fallback만 DB와 Supertonic·MuseTalk 입력으로 사용한다.
