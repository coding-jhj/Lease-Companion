...
- 마지막 갱신: 2026-07-22
- 현재 완료 범위: 작업 0~8, 작업 9 전용 E2E 구현·MSW 브라우저 사전검증
- 현재 브랜치: `main`
- 마지막 커밋: `a64b69c docs(practice): add real API validation guide`
- 작업 7 커밋: `332f425 feat(practice): add frontend simulation flow`
- 작업 8 상태: 구현·검증·커밋 완료
- 작업 9 상태: 전용 Playwright·실행 명령 구현, MSW `9 passed`, 실제 API 실행 대기
- 다음 작업: PostgreSQL·Backend·Frontend를 실행한 작업 9 실제 API 검증
...
현재는 **시나리오 3개, 기존 R/J/DP 연결, 사용자 답변 평가, 누적 상태 머신, Gemini/Fake provider 선택, Backend 세션·턴·결과 저장 API, Frontend 네 화면, Frontend 단위·통합 테스트, 전용 Playwright E2E까지 구현한 상태**입니다. MSW 브라우저 사전검증은 3개 viewport에서 `9 passed`이며, 실제 Frontend↔Backend E2E 실행과 Gemini 네트워크 smoke는 아직 완료되지 않았습니다.
...
| 작업 9 | 세 시나리오 전용 Playwright E2E | 실행 준비 완료 | MSW `9 passed`, 실제 API 명령 준비 |
| 작업 10 이후 | 실제 Frontend↔Backend 실행·전체 회귀 | 예정 | 실제 API 미실행 |
...
`FakePracticeProvider`는 승인된 `answer-key.json` 예시를 먼저 결정적으로 재생합니다. 예시에 없는 답변은 승인된 세 시나리오 9개 TURN에 한해 필수 의미 키워드 묶음을 검사합니다. 필수 의미를 모두 표현하면 `appropriate_check`로 진행하고, 무관하거나 먼저 송금·서명하겠다는 상충 표현은 `needs_review`로 같은 TURN에 남깁니다. 범용 자연어 모델이 아니므로 세 시나리오 밖의 의미를 추측하지 않으며 네트워크를 사용하지 않습니다.
...
- 작업 9 전용 브라우저 E2E는 구현됐으며, Fake provider 기반 실제 Frontend↔Backend 실행만 남았습니다.
...
- 모바일 320px·360px의 시나리오 1열 배치와 PC 카드 배치는 Playwright로 확인했습니다.
- Practice 전용 키보드 순서 검증은 아직 별도 항목으로 고정하지 않았습니다.
- 실제 Gemini 유료 호출은 계속 제외합니다.

## 17. 작업 9 — 전용 Playwright E2E와 실제 API 실행 준비

### 작업 목적

세 시나리오를 브라우저에서 동일한 순서로 반복 검증하고, MSW 사전검증을 통과한 같은 파일을 실제 FastAPI·PostgreSQL 검증에도 사용합니다.

### 변경한 파일과 역할

| 파일 | 역할 |
|---|---|
| `frontend/e2e/practice-flow.spec.ts` | 세 시나리오의 회원가입부터 저장된 복기까지 실제 브라우저로 검증 |
| `frontend/package.json` | `test:e2e:practice`, `test:e2e:practice:real` 전용 명령 제공 |
| `frontend/src/mocks/practice.ts` | MSW 세션을 새로고침 뒤에도 복원하고 특약 수정 요구를 확인 행동으로 평가 |
| `frontend/src/pages/practice/PracticeSessionPage.tsx` | 진행 요약을 접근 가능한 `status` 영역으로 제공 |
| `ai/src/lease_companion_ai/providers/gemini_practice.py` | Fake provider가 세 시나리오의 자연스러운 필수 의미 답변을 결정적으로 판별 |
| `ai/tests/providers/test_gemini_practice.py` | 자연 답변 9개 통과와 상충 답변 3개 거부를 검증 |
| `backend/tests/api/test_practice.py` | 실제 API에서 세 시나리오 자연 답변이 TURN-02로 진행되는지 검증 |
| `scripts/start-dev.ps1` | `-PracticeValidation` 한 명령으로 DB·Backend·Frontend 준비 후 브라우저 실행 |
| `docs/testing/practice-real-api-validation.md` | 한 명령 실행과 화면 검증 순서를 사용자 기준으로 설명 |

### 자동 확인 흐름

1. 독립 계정 회원가입·로그인
2. 대시보드에서 연습 목록 진입
3. 세 시나리오와 모바일·PC 배치 확인
4. 합성 계약 상황과 특약 확인
5. 미응답 후 같은 TURN 재시도
6. 승인 답변으로 3개 TURN 진행
7. TURN-01 뒤 새로고침하여 TURN-02와 누적 행동 복원
8. 최종 행동 `보류` 저장
9. 확인 행동·놓친 신호·공식자료 ID 복기
10. 결과 새로고침 후 같은 결과 재조회

### 검증 결과

- `npm run test:e2e:practice`: `9 passed`
- 자연 답변 수정 관련 AI·Backend 회귀: `295 passed`
- Ruff·mypy: 통과
- 대상: 시나리오 3개 × 320×720·360×800·1440×900
- 실제 Backend 호출: `0회` — 이 결과는 MSW 브라우저 사전검증임
- 실제 Gemini 호출: `0회`

### 바로 실행할 실제 API 검증

Docker Desktop을 켠 뒤 저장소 루트에서 명령 하나를 사용합니다.

```powershell
pwsh -File .\scripts\start-dev.ps1 -PracticeValidation
```

DB·migration·Fake provider·Backend·Frontend 준비 후 회원가입 화면이 자동으로 열립니다. 자동 Playwright 실제 API 검증은 서버가 열린 상태에서 `npm run test:e2e:practice:real`을 추가 실행합니다. 자세한 화면 순서는 [계약 연습 실제 API 검증 방법](../testing/practice-real-api-validation.md)을 따릅니다.
...
| 2026-07-22 | 작업 9 전용 Playwright E2E 준비 | 미커밋 | 세 시나리오×3 viewport `9 passed`, 실제 API 실행 명령 준비 |
