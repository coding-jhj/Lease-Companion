...
- 현재 완료 범위: 작업 0~7
- 현재 브랜치: `main`
- 마지막 커밋: 작업 6 커밋 `49fd104 test(practice): cover backend API flows`
- 작업 7 상태: 구현·기존 회귀 검증 완료, 아직 미커밋
- 다음 작업: 작업 8 — 세 시나리오 Frontend 전용 테스트
...
현재는 **시나리오 3개, 기존 R/J/DP 연결, 사용자 답변 평가, 누적 상태 머신, Gemini/Fake provider 선택, Backend 세션·턴·결과 저장 API, Frontend 목록·상황·대화·복기 화면과 MSW 연결까지 완성한 상태**입니다. Frontend 전용 테스트와 실제 Frontend↔Backend E2E, 실제 Gemini 네트워크 smoke는 아직 완료되지 않았습니다.
...
| 작업 8 이후 | Frontend 전용 테스트·실제 E2E | 예정 | 미구현 |
...
flowchart TD
    ...
    M -. 후속 작업 .-> O[Frontend 전용 테스트와 실제 E2E]
```

실선은 작업 0~7에서 준비되거나 연결된 범위입니다. 점선은 아직 남은 작업입니다.
...
- 작업 8에서 세 시나리오 전용 화면·서비스 테스트를 추가해야 합니다.
...
| 2026-07-22 | 작업 7 Frontend 화면·서비스·MSW | 미커밋 | 세 시나리오 공통 흐름, 기존 Frontend 75개·build 통과 |
