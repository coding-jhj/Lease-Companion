...
현재 병합 기준: **AI·Backend 745(2 skipped) · Frontend 100** 통과.
...
| # | 자료 | 파일 | 상태 |
|---|------|------|------|
| 1 | 특약 카탈로그 정답 정의 | `data/rules/special_clause_catalog.json` | 게이트 A 데이터 지적사항 반영 ✅ / **규칙 구조 수정·독립 재확인 필요**(4절) |
...
| 작업 | 내용 | 의존 |
|------|------|------|
| 작업 3 | R08·R09·R10·R19와 J10·J11·J12 책임 경계 ADR | 작업 2 |
...
2. **작업 3 R/J 경계 ADR** — 기존 R01~R24 의미를 유지하면서 J10·J11·J12와 R08·R09·R10·R19의 책임 경계를 고정한다.
3. 작업 4 규칙 보완 → 5 RAG → 6 생성·guardrail → 7 AI 종단 → 8 Backend → 9 Frontend → 10 오프라인 종단 검증 순서로 구현한다.
