...
## 6. 다음 담당자 착수 우선순위 (권장)
1. **게이트 B 평가셋 잠금·자체 검토** — 혼자 작업하는 동안 `draft_pending_human_review`를 유지하고 독립 검토는 운영 활성화 전에 수행한다.
2. **카탈로그/규칙 구조 결정 기록** — 기존 R01~R24 의미를 유지하면서 J10·J11·J12와 R08·R09·R10·R19의 책임 경계를 ADR로 고정한다.
3. Task 4(특약 RAG) → 5 → 6 → 7 → 8 → 9.
