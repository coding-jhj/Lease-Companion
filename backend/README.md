...
- **canonical v1.2.0 연결 완료(2026-07-17)**: ① confirm이 계약 상황을 `ContractContext`로 스냅샷에 불변 포함(필수값 미입력 시 422 `missing_contract_context`, 응답 `snapshot`에 canonical payload 전체) ② 분석 시작 시 스냅샷과 현재 계약 상황 비교 — 다르면 422 `contract_context_changed`(기존 스냅샷은 불변, 재confirm으로 새 스냅샷) ③ 워커가 규칙 결과 저장 후 GenerationService·Guardrail 실행 — guardrail 통과 결과만 `generation_result`에 분리 저장, provider 키 없으면 template fallback으로 정상 완료(generation_status=completed, 항목별 generation_method로 구분), 생성 실패 시 규칙 결과 유지 + generation_status=failed·안전 문구만 generation_error에 저장.
