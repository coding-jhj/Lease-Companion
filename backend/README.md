...
- **classification 실행·저장 연결 완료**: 분석 worker가 `ClassificationService`를 호출하고 canonical `ClassificationResult`를 `analysis_runs.classification_result`에 저장한다. API에는 노출하지 않는다. provider 실패는 `safe_fallback`과 `fallback_reason_code`로 기록하며 `classification_error`는 중복 저장하지 않고 `null`을 유지한다.
