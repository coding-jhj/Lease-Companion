...
- **생성 결과 저장 준비(2026-07-17 3인 합의)**: `AnalysisRun.generation_result` 컬럼·API 응답 필드 추가 — 규칙 판정(result)과 분리, guardrail 통과 결과만 저장 예정. 워커의 생성 호출 연결과 `contract_context` 스냅샷 포함은 **A의 canonical schema v1.2.0 확정 후** 진행.
